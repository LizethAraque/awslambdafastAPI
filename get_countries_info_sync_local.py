...
#Debe modularizar el codigo en una fracion mas pequeña, por eso no se debe dejar todo el codigo en esta. Este es el orquestador de toda la lambda.  
def lambda_handler(event, context):
	return asyncio.run(run_countries_info(event, context))

#Este es el despliegue al servidor a traves de uvicorn, cuando se hace el despliegue en AWS este no es necesario
#if __name__=="__main__":
#    PORT = int(os.environ.get('PORT', 8080))
#    uvicorn.run("get_countries_info_sync:app", host='127.0.0.1', port=PORT ,reload=True)
